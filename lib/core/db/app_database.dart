...
class AppDatabase {
  ...
  Future<void> _createVisitTables(Database database) async {
    ...
    await database.execute(
      'CREATE UNIQUE INDEX IF NOT EXISTS idx_visit_voice_notes_tenant_uuid '
      'ON local_visit_voice_notes(tenant_id,client_uuid)',
    );
    await database.execute(
      'CREATE INDEX IF NOT EXISTS idx_visit_voice_notes_visit '
      'ON local_visit_voice_notes(tenant_id,visit_offline_uuid,sync_status)',
    );
